...
TiseOcx=Ocx(Time)*10;   % 脱分極バンド中心のX座標
Ocy=R.*cos(Ac);         % 脱分極バンド中心のY座標
Ocz=R.*sin(Ac);         % 脱分極バンド中心のZ座標

OOc=sqrt(TiseOcx^2+Ocy^2+Ocz^2);

% Ocq=sqrt((Qx-Ocx*10).^2+(Qy-Ocy).^2+(Qz-Ocz).^2);    % 脱分極バンドの中心から任意点までの距離
OcQ=sqrt((Qx-TiseOcx).^2+(Qy-Ocy).^2+(Qz-Ocz).^2);    % 脱分極バンドの中心から任意点までの距離

Pq=acos((OOc.^2+OQ.^2-OcQ.^2)./(2.*OOc.*OQ));
end